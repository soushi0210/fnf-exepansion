...
local textureSplash = 'BloodSplashes';
...
function onCreatePost()
	if string.match(songName,'endless') == 'endless-encore' then
		enableNewSystem = 0
		precacheImage('FunInfiniteStage/endlessNoteSplashes')
	end
	if songName == 'Too-Fest' then
		precacheImage('hitmarker')
		precacheSound('Sanic/hitmarker')
	end
...
function onStepHit()
	if curStep == 900 and songName == 'endless' or curStep == 800 and songName == 'endless-encore' then
		enableNewSystem = 2
		textureSplash = 'FunInfiniteStage/endlessNoteSplashes'
		defaultSplashX = 135
		defaultSplashY = 105
	end
	if songName == 'Too-Fest' then
